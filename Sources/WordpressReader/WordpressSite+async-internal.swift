...
extension WordpressSite {
    /// Fetches the starting page and returns it with page numbers and URLs for the remaining pages.
    /// - Parameter request: Request used to retrieve paginated items.
    /// - Returns: The starting page, when available, and page numbers and URLs for the remaining pages.
    ...
    nonisolated func fetchPagination<T: WordpressItem>(
    ...
    ) async throws -> (firstPage: (number: Int, batch: [T])?, remainingPages: [(number: Int, url: URL)]) {
        ...
        var firstPage: (number: Int, batch: [T])?
        
        if let page = request.queryItems.pages.first {
            pageRange = page...page
        } else {
            urlComponents.queryItems = request.urlQueryItems(request.startPage)
            ...
            firstPage = (request.startPage, result.value)

            guard let header = result.response.value(forHTTPHeaderField: Self.totalPagesHeader) else {
